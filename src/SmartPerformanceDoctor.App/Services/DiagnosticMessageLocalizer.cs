...
public static class DiagnosticMessageLocalizer
{
    ...
    public static string Localize(string? message)
    {
        ...
        if (text.EndsWith(" 시작", StringComparison.Ordinal))
        {
            var id = text[..^3].Trim();
            if (StageLabels.TryGetValue(id, out var startLabel))
            {
                return $"{startLabel} 중…";
            }
        }

        var completeMatch = Regex.Match(text, @"^(.+?) 완료: (.+)$");
        if (completeMatch.Success)
        {
            var id = completeMatch.Groups[1].Value.Trim();
            var status = completeMatch.Groups[2].Value.Trim();
            var stage = StageLabels.TryGetValue(id, out var label) ? label : HumanizeId(id);
            var statusKo = StatusLabels.TryGetValue(status, out var s) ? s : status;
            return $"{stage} 완료 ({statusKo})";
        }

        if (StageLabels.TryGetValue(text, out var direct))
        {
            return direct;
        }
