...
- 홈 상태 카드의 텍스트 대비를 높이고 밝은 전용 색상 토큰으로 통일했습니다.
- 홈의 `정밀 점검·복구 시작` 버튼이 전체 진단 후 발견된 문제의 복구까지 한 흐름으로 실행되도록 승인 전달을 수정했습니다.
- 보안 금고와 보안 삭제의 임시 도형 문자를 Fluent 자물쇠·삭제 아이콘으로 교체했습니다.
- 보안 금고·보안 삭제·업데이트의 파일 선택기를 Windows App SDK 2.2 공통 서비스로 통합하고 파일 다중 선택과 폴더 선택을 지원합니다.
- 선택 성공·취소·실패를 분리하고, 중복 선택 창 차단·포커스 복원·비식별 오류 추적을 적용했습니다.
- 보안 삭제의 보호 경로와 재분석 지점 차단을 강화하고, 차단된 드라이브 전체를 불필요하게 순회하던 Dry Run 성능 문제를 제거했습니다.
- 소스 루트에 섞여 있던 실행 DLL·XBF·언어 리소스·Rust 산출물을 제거하고 런타임 게시 위치를 `artifacts/runtime/`으로 분리했습니다.
- 제품에서 사용하는 SecurityLab을 `src/`, 테스트를 `tests/`, CLI를 `tools/`로 재배치하고 솔루션에 정식 등록했습니다.
- 전체 솔루션 빌드, 576개 회귀 테스트, 36개 SecurityLab 테스트와 self-contained 런타임 게시·무결성 검증을 통과했습니다.
...
51.0.2 x64 Release 솔루션 빌드는 **경고 0개 / 오류 0개**로 통과했습니다.

- 시스템 케어·진행률·업데이트 정리·설치 수명주기 집중 테스트: **33 passed / 0 failed**
- 전체 회귀 스위트: **576 passed / 116 skipped**
- SecurityLab 보안 테스트: **36 passed / 0 failed**
...
| `src/SmartPerformanceDoctor.SecurityLab/` | 보안 금고·보안 삭제 보강 계층 |
| `tests/` | 단위·계약·회귀 테스트 |
| `scripts/` | 빌드·게시·패키징·검증 스크립트 |
| `content/` | 규칙·데이터·아이콘 등 제품 콘텐츠 |
| `tools/` | 개발 및 검증용 CLI 도구 |
| `artifacts/runtime/` | 빌드 시 생성되는 실행 배포본(소스와 분리, Git 제외) |
...
저장소 최상단에는 소스 구성 파일만 두며, 실행 DLL·XBF·언어 리소스와 Rust `target/` 산출물은 커밋하지 않습니다. 자세한 규칙은 [저장소 폴더 규칙](docs/architecture/repository-layout.md)을 참고하세요.
